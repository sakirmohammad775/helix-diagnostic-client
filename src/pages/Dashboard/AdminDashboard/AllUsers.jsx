...
const AllUsers = () => {
  ...
  const handleMakeAdmin = (userId, userName) => {
    Swal.fire({
      ...
      text: `Are you sure you want to make ${userName || "this user"} an Admin?`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#7c3aed", // Purple color matching your button
      cancelButtonColor: "#64748b",
      confirmButtonText: "Approve",
      cancelButtonText: "Cancel",
    }).then((result) => {
      if (result.isConfirmed) {
        // Execute TanStack Mutation when Approved
        roleMutation.mutate(userId, {
          onSuccess: () => {
            Swal.fire({
              title: "Promoted!",
              text: `${userName || "User"} is now an Admin.`,
              icon: "success",
              timer: 2000,
      ...
      <div className="flex justify-between items-center border-b border-gray-100 pb-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage patient accounts, access permissions, and roles.
          </p>
        </div>
              ...
              return (
                ...
                >
                  <td className="py-4 px-4">
                    <Link
                      to={`/dashboard/user-details/${encodeURIComponent(user.email)}`}
                  ...
                  <td className="py-4 px-4 text-right space-x-2">
                    ...
                    {!isAdmin && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-3 py-1.5 bg-purple-50 text-purple-700 hover:bg-purple-100 rounded-lg text-xs font-medium transition cursor-pointer"
                      >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl relative space-y-5">
            ...
            <h3 className="text-xl font-bold text-gray-900 border-b border-gray-100 pb-3 flex items-center gap-2">
              <UserCheck className="w-5 h-5 text-blue-600" /> User Profile
              Information
            </h3>
